...
class Predicting:
    ...
    def exc(self, master: mr.Master, model: tfc.Sum, v_posterior_samples: collections.OrderedDict) -> pd.DataFrame:
        ...
        data = pd.concat((master.training['n_attendances'], master.testing['n_attendances']), ignore_index=True).values
        data = np.concat((data, np.repeat(np.nan, self.__arguments.get('ahead'))))

        # Times
        times = self.__times(start=master.training['week_ending_date'].min(), periods=data.shape[0])
