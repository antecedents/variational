...
class Architecture:
    ...
    def __model(self, _training: np.ndarray) -> tfc.Sum:
        ...
        trend = tfp.sts.LocalLinearTrend(observed_time_series=_training)

        week_of_year_effect = tfp.sts.Seasonal(
        ...
        model = tfp.sts.Sum([trend, week_of_year_effect, autoregressive],
                            observed_time_series=_training)
    ...
    def exc(self, master: mr.Master) -> ifr.Inference:
        ...
        # Model
        model: tfc.Sum = self.__model(_training=master.training['n_attendances'].values)

        # Posteriors & Evidence Lower Bound
        v_posterior, v_elb = self.__variational(model=model, _training=master.training['n_attendances'].values)
